 //services/ProduceCategoryService.j
 const MongooseService = require("./MongooseService"); // Data Access Layer
 ProduceCategoryModel = require('../models/produceCategory'); // Database Model
 
 module.exports = class ProduceCategoryService {
   /**
   ...
   async create ( documentToCreate ) {
    ...
    async get ( query) {
      try {
          ...
          if(result.length == 0)
            throw {error: 404, message: "ProduceCategory not found"};
   ...
   async getById (id) {
    try {
      const result = await this.MongooseServiceInstance.findById();
      return { success: true, body: result}; 
  } catch (error) {
      return { success: false, err: error };
  }
