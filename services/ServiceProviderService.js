 //services/ServiceProviderService.j
 const MongooseService = require("./MongooseService"); // Data Access Layer
 ServceProviderModel = require('../models/serviceProvider'); // Database Model
 
 module.exports = class ServiceProviderService {
   /**
    * @description Create an instance of ServiceProviderService
    */
   constructor () {
     // Create instance of Data Access layer using our desired model
     this.MongooseServiceInstance = new MongooseService( ServiceProviderModel );
   }
   ...
   async create ( documentToCreate ) {
    ...
    async get ( query) {
      try {
          ...
          if(result.length == 0)
            throw {error: 404, message: "ServiceProvider not found"};
    ...
    async update ( id, userToUpdate ) {
      try {
          const result = await this.MongooseServiceInstance.update( id, userToUpdate);
          return { success: true, body: result };
      } catch ( error ) {
          return { success: false, err: error };
      }
